...
class Graph:
    ...
    def __init__(self, system_prompt: str, retry_prompt: str,
                 converter_agent: Callable[[State, BaseChatModel], State],
                 validator_agent: Callable[[State, BaseChatModel], State],
                 router_agent: Callable[[State], State],
                 ollama_model: str = '', openai_model: str = '', id: str = ''):
        # Global state to maintain conversation across calls
        self.id = id
        self.conversation_state = State(
            messages=[], message_type="valid", errors=[])
    ...
    def __build_graph(self):
        ...
        graph_builder.add_node("router", self.router_agent)
    ...
    def bound_validator_agent(self, state: State) -> State:
        return self.validator_agent(state, self.llm)

    def bound_converter_agent(self, state: State) -> State:
        return self.converter_agent(state=state, llm=self.llm)
