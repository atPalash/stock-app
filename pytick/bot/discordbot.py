...
Or use: Feature → Scenario → Given/When/Then",
    ...
    async def __handle_backtest(self, interaction: discord.Interaction, queries: list[str], start: int, stop: int, stop_loss_percent: float, commission: float, timeout: int):
    ...
    async def __do_backtest(self, message: str, interaction: discord.Interaction, queries: list[str], start: int, stop: int, stop_loss_percent: float, commission: float, timeout: int) -> RetVal:
        try:
            ...
            await asyncio.wait_for(
                self.__handle_backtest(interaction=interaction, queries=queries, start=start, stop=stop, stop_loss_percent=stop_loss_percent, commission=commission, timeout=timeout),
                timeout=float(timeout)
            )
            return RetVal(status=True, message="")
        except asyncio.TimeoutError:
            logger.warning(f"Backtest timeout: {queries}")
            return RetVal(status=False, message=f"⏰ Cannot complete backtest for within {timeout}s", errors=["Timeout"])
        except Exception as e:
            logger.warning(f"{e}")
            raise e
