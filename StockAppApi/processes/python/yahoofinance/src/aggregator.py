...
class Aggregator:
    def __init__(self, indicator_config_file: str, selected_stocks_yaml) -> None:
        ...
        for interval in read_config(self.indicator_indicator_config_file)['indicator']['data']:
            ...
            if interval == 'week':
                ...
                scheduler.add_job(self.__periodic_download, 'cron', hour='9-16', minute='16',
                                  day_of_week='mon-fri', timezone=pytz.timezone('Asia/Kolkata'), args=[interval])
                #  Test stub
                # scheduler.add_job(self.__periodic_download, 'cron', second='*', timezone=pytz.timezone('Asia/Kolkata'), args=[interval])
                scheduler.start()
            else:
    ...
    def __periodic_download(self, interval: str):
        ...
        download_historical_data(tickers=tickers,
                                 period=periods_[yahoo_interval],
