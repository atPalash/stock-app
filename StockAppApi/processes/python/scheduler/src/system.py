...
class SystemScheduler(Scheduler):
    ...
    def run(self):
        ...
        scheduler.add_job(self.__hourly_elder_impulse, 'cron', hour='8-16', minute='20',
    ...
    def __hourly_elder_impulse(self):
        ...
        try:
            ...
            if self.elder_impulse_df is not None:
                ...
                diff_rows = self.elder_impulse_df[self.elder_impulse_df['trend'].
                                                  ne(res['result']['trend'])]
                requests.post(self.master_url, data=message + f"--title ElderImpulse --message {diff_rows.to_string()}")
            else:
                self.elder_impulse_df = res['result']
            ...
            if now_in_kolata.time() < target_time:
                if res['status']:
                    df = self.elder_impulse_df.to_string()
