...
import pandas
from StockAppApi.base.python.src.json_helper import read_json
...
class YahooFinance(System):
    def __init__(self, indicator_config_file: str, selected_stocks_config_file: str,
        ...
        self.interval = {'minute': '1m', 'minute5': '5m',
    ...
    def __get_fundamentals(self, debug_tickers: list = []) -> RetVal:
        ...
        error = download_stock_stats(
            tickers=tickers, destination=self.indicator_config['indicator']['fundamental'])
                ...
                for data in statements:
                    for date, statement in data.items():
                        temp[datetime.strptime(
                            date, "%Y-%m-%d")] = statement
                ret[statementType] = dict(sorted(temp.items())) # sort by date.python dicts are not sorted
            return RetVal(obj=ret, obj_as_str=f'{self.parameter["ticker"]}: financials dict', errors="")
    ...
    def __get_ohlc(self) -> RetVal:
        ...
        try:
            ...
            return RetVal(obj=ticker_df, obj_as_str=f'{self.parameter["ticker"]}: financials dict', errors="")
        except Exception as e:
            raise
...
if __name__ == "__main__":
    ...
    yf = YahooFinance(indicator_config_file=indicator_config_yaml,
                      ...
                      parameter={'interval': 'day', 'panda': 1, 'ticker': 'ABB'}, command_handler=None,
                      name="")
    data = yf.debug_get()
    print(data)
