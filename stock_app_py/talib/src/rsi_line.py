...
from stock_app_py.yahoofinance.src.data_fetcher import download_historical_data
...
class RsiLine(Indicator):
    ...
    def _do_analysis(self, latest=True):
        # index data
        index_ohlc, err = download_historical_data(
            tickers=self.parameter["index"],
        ...
        strength_series = (strength_series - strength_series.min()) / (
            strength_series.max() - strength_series.min()
        )
