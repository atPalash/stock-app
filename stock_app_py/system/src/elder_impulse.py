...
class ElderImpulse(System):
    def __init__(
        ...
        self.commands = {"get": self.__get}  # calls with all or list of tickers
    ...
    def __get(self) -> RetVal:
        ...
        for stock in self._get_indices() + self._get_tickers():
            try:
                ...
                ema = self.command_handler.execute(ema_day_query, is_rest=False).obj[
                    "ema"
                ]
                slope_ema_day, _, _, _, _ = linregress(
                    numpy.arange(0, ema.shape[0], 1), ema
                )
                ...
                ema = self.command_handler.execute(ema_week_query, is_rest=False).obj[
                    "ema"
                ]
                slope_ema_week, _, _, _, _ = linregress(
                    numpy.arange(0, ema.shape[0], 1), ema
                )
                ...
                macdhist = self.command_handler.execute(
                    macdhist_day_query, is_rest=False
                ).obj["macdhist"]
                slope_macdhist_day, _, _, _, _ = linregress(
                    numpy.arange(0, macdhist.shape[0], 1), macdhist
                )
                ...
                {self.parameter["macd_signal_period"]} --n 2 --latest {self.parameter["latest"]}'  # get latest 2 macdhists
                macdhist = self.command_handler.execute(
                    macdhist_week_query, is_rest=False
                ).obj["macdhist"]
                slope_macdhist_week, _, _, _, _ = linregress(
                    numpy.arange(0, macdhist.shape[0], 1), macdhist
                )

                ema_impulse = (1 if slope_ema_day > 0 else -1) + (
                    1 if slope_ema_week > 0 else -1
                )
                macd_impulse = (1 if slope_macdhist_day > 0 else -1) + (
                    1 if slope_macdhist_week > 0 else -1
                )
                trend = "no trend"
