        ...
        for ticker in self._get_tickers():
            ...
            try:
                rsiline_query = f'talibquery --ticker {ticker} --interval {self.parameter["interval"]} --do get \
                    ...
                    --panda 1'
                rsiline = self.command_handler.execute(
                    rsiline_query, is_rest=False
                ).obj["rsiline"]

                folder_path = self.indicator_config["indicator"]["fundamental"]
                quarterly_financials = pandas.read_csv(
                    f"{folder_path}/{ticker}_quarterly_financials.csv",
                    index_col=0,
                    parse_dates=True,
                )
                quarterly_balancesheet = pandas.read_csv(
                    f"{folder_path}/{ticker}_quarterly_balancesheet.csv",
                    index_col=0,
                    parse_dates=True,
                )
                financials = pandas.read_csv(
                    f"{folder_path}/{ticker}_financials.csv",
                    index_col=0,
                    parse_dates=True,
                )
