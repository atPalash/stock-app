        ...
        for stock in self._get_indices() + self._get_tickers():
            try:
                ...
                def getMa(window):
                    ...
                    ma_list = self.command_handler.execute(query, is_rest=False).obj[
                        self.parameter["stage2scannertype"]
                    ]
                ...
                valid, pos = shift(
                    latest_price > latest_ma_150 and latest_price > latest_ma_200,
                    valid,
                    pos,
                )
                ...
                valid, pos = shift(
                    latest_ma_50 > latest_ma_150 and latest_ma_50 > latest_ma_200,
                    valid,
                    pos,
                )
                valid, pos = shift(
                    latest_price > (low_52_week + low_52_week * 0.25), valid, pos
                )
                valid, pos = shift(
                    latest_price > (high_52_week - high_52_week * 0.25), valid, pos
                )
