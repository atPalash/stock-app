    ...
    def __get(self) -> RetVal:
        ...
        for ticker in self._get_tickers() + self._get_indices():
            try:
                ...
                for i in range(
                    ret.index.min(), ret.index.max() - self.parameter["window"] + 1, 1
                ):
                    ...
                    if window_macdhist_min < 0 and window_macdhist_max > 0:
                        ...
                        if window_macdhist_min_index < window_macdhist_max_index:
                            sub_window = window.loc[
                                window_macdhist_max_index : i + self.parameter["window"]
                            ]
                            ...
                            # min on right of max
                            if (
                                sub_window_macdhist_min < 0
                                and sub_window_macdhist_max > 0
                            ):
                                ...
                                priceA = window[self.parameter["ohlc"]].loc[
                                    window_macdhist_min_index
                                ]
                                priceC = window[self.parameter["ohlc"]].loc[
                        ...
                        if window_macdhist_min_index > window_macdhist_max_index:
                            sub_window = window.loc[
                                window_macdhist_min_index : i + self.parameter["window"]
                            ]
                            sub_window_macdhist = sub_window["macdhist"]
                            sub_window_macdhist_min = sub_window_macdhist.min()
                            sub_window_macdhist_max = sub_window_macdhist.max()
        ...
        return RetVal(
            obj=divergence_df,
            obj_as_str=f"sample\n{divergence_df[ticker].to_string(max_rows=None, max_cols=None, index=False)}",
            errors=errors,
        )
